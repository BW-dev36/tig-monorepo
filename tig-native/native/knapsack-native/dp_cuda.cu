...
#include <iostream>
// Outputs the solution to the `taken_indices` array.
template <typename T>
                  ...
                  const index_t num_items)
{
    ...
    index_t taken_count = 0;
    for (index_t idx = num_rows - 1; idx > 0; --idx) {
    ...
    // Terminate the list with a special value indicating the end (optional)
    //taken_indices[taken_count] = static_cast<uint32_t>(-1);
}
...
using cuda_unique_ptr =
    std::unique_ptr<typename std::remove_pointer<T>::type, cudaDeleterWrapper<D, fn>>;
...
{
    int nb_gpu = 0;
    GPU_ERRCHECK(cudaGetDeviceCount(&nb_gpu));
    return nb_gpu;
}
                     ...
                     uint32_t* taken_indices)
{
    ...
    GPU_ERRCHECK(cudaSetDevice(gpu_to_use));
    // GPU memory buffers that each hold at least (`capacity` + 1) elements of
    // type `value_t`.
    auto dev_buffer_a = create_gpu_buffer<value_t>(gpu_buf_size);
    auto dev_buffer_b = create_gpu_buffer<value_t>(gpu_buf_size);
    ...
    // Host memory buffer to hold the full solution.
    using backtrack_t = uint32_t;
    auto backtrack = create_backtrack_matrix<backtrack_t>(num_items, capacity_plus_one);
    ...
    // first item
    {
        size_t second_stream_index = std::min((size_t)1, (size_t)num_streams - 1);
        GPU_ERRCHECK(cudaMemsetAsync(dev_backtrack.get(), 0, gpu_buf_size, streams[0].get()));
        GPU_ERRCHECK(cudaMemsetAsync(prev, 0, gpu_buf_size, streams[second_stream_index].get()));
    }
    ...
    for (index_t i = 0; i < num_items; ++i) {
        ...
        for (index_t j = 0; j < num_streams; ++j) {
            ...
            dynamic_prog<<<NUM_SEGMENTS, NUM_THREADS, 0, stream>>>(prev, curr, dev_backtrack.get(), weight, value, j * TOTAL_THREADS);
            ...
            if (i % num_bits == (num_bits - 1) || i == num_items - 1) {
                ...
                cudaMemcpyAsync(backtrack.get() + idx * capacity_plus_one + j * TOTAL_THREADS,
                                dev_backtrack.get() + j * TOTAL_THREADS,
                                std::min(TOTAL_THREADS, capacity_plus_one - j * TOTAL_THREADS) * sizeof(backtrack_t),
                                cudaMemcpyDeviceToHost, stream);
            }
        }
    ...
    // If `num_items` is even, the last output is in A, otherwise it's in B
    value_t* ptr = (num_items % 2) ? dev_buffer_a.get() : dev_buffer_b.get();
    GPU_ERRCHECK(cudaMemcpyAsync(&best, ptr + capacity, sizeof(value_t), cudaMemcpyDeviceToHost));
