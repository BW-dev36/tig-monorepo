...
pub fn native_compute_solution(
    ...
    let serialized_challenge = match settings.challenge_id.as_str() {
        ...
        "c004" => {
            let vec_diff =  &settings.difficulty;
            let vs_difficulty = tig_native::vector_search::VSODifficulty {
                num_queries: vec_diff.get(0).unwrap().clone() as u32,
                better_than_baseline: vec_diff.get(1).unwrap().clone() as u32,
            };

            let vsochallenge = unsafe { tig_native::vector_search::generate_instance_vs(seeds.as_ptr(), &vs_difficulty) };

            let challenge = tig_challenges::vector_search::convert_vsochallenge_to_challenge(vsochallenge);
            unsafe { tig_native::vector_search::free_vso_challenge(vsochallenge); }
...
pub fn native_verify_solution(
    ...
    match settings.challenge_id.as_str() {
        ...
        "c004" => {
            ...
            let challenge =
            tig_challenges::vector_search::convert_vsochallenge_to_challenge(
                unsafe { tig_native::vector_search::generate_instance_vs(seeds.as_ptr(), &vs_difficulty) });
            match tig_challenges::vector_search::Solution::try_from(solution.clone()) {
                Ok(solution) => challenge.verify_solution(&solution),
                Err(_) => Err(anyhow!(
